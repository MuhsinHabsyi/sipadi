<x-sipadi-layout title="Tambah Anggota Petani">

<div class="card" style="max-width: 720px; margin: 0 auto;">
    <div class="card-header" style="border-bottom: 1px solid var(--color-border); padding-bottom: 16px; margin-bottom: 24px;">
        <h2 class="card-title" style="font-size: 20px;">Form Pendaftaran Anggota Tani</h2>
        <p style="font-size: 13px; color: var(--color-muted); margin-top: 4px;">Lengkapi data identitas diri dan detail lahan garapan petani di bawah ini.</p>
    </div>
    ...
    <form method="POST" action="{{ route('anggota.store') }}">
        ...
        <div style="margin-bottom: 32px; padding: 20px; background: #F8FAFC; border-radius: var(--radius-md); border: 1px solid #E2E8F0;">
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 16px; margin-bottom: 16px;">
                ...
                <div>
                    ...
                    <input type="number" name="umur" value="{{ old('umur') }}" min="1" max="150"
            ...
            <div>
                ...
                <textarea name="alamat" rows="3" placeholder="Masukkan alamat domisili petani"
                          style="width: 100%; padding: 10px 14px; border: 1.5px solid var(--color-border); border-radius: var(--radius-md); font-family: inherit; font-size: 14px; background: white; resize: vertical;">{{ old('alamat') }}</textarea>
        ...
        <div style="margin-bottom: 32px; padding: 20px; background: linear-gradient(135deg, #F0F7F4, #E2EFEA); border-radius: var(--radius-md); border: 1px solid #B7E4C7;">
            ...
            <div style="display: grid; grid-template-columns: 3fr 2fr; gap: 16px; margin-bottom: 16px;">
                <div>
                    ...
                    <input type="text" name="nama_lahan" value="{{ old('nama_lahan') }}"
                           placeholder="Contoh: Blok Sawah Kidul"
                           style="width: 100%; padding: 10px 14px; border: 1.5px solid var(--color-border); border-radius: var(--radius-md); font-family: inherit; font-size: 14px; background: white;">
                ...
                <div>
                    ...
                    <select name="status_lahan"
                        ...
                        @foreach(\App\Models\AnggotaPetani::STATUS_LAHAN_OPTIONS as $key => $label)
                            <option value="{{ $key }}" {{ old('status_lahan') == $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
            ...
            <div style="margin-bottom: 16px;">
                ...
                <input type="number" step="0.1" name="luas_lahan" value="{{ old('luas_lahan') }}" required
                       ...
                       style="width: 100%; padding: 10px 14px; border: 1.5px solid var(--color-border); border-radius: var(--radius-md); font-family: inherit; font-size: 14px; background: white;">
                <span style="font-size: 11px; color: var(--color-muted); margin-top: 4px; display: block;">1 tumbak setara dengan ±14 meter persegi.</span>
            ...
            <div>
                ...
                <textarea name="alamat_lahan" rows="2" placeholder="Masukkan alamat spesifik lokasi lahan garapan"
                          style="width: 100%; padding: 10px 14px; border: 1.5px solid var(--color-border); border-radius: var(--radius-md); font-family: inherit; font-size: 14px; background: white; resize: vertical;">{{ old('alamat_lahan') }}</textarea>
        ...
        <div style="display: flex; gap: 12px; justify-content: flex-end;">
            ...
            <button type="submit" style="padding: 10px 16px; background: var(--color-primary); color: black; border: none; border-radius: var(--radius-md); font-size: 14px; font-weight: 600; cursor: pointer; transition: opacity .2s;" onmouseover="this.style.opacity='.9'" onmouseout="this.style.opacity='1'">Simpan Data Anggota</button>
        </div>
    </form>
</div>
